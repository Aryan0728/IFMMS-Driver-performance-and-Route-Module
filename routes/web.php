...
Route::prefix('driver')->name('driver.')->middleware(['auth', RoleMiddleware::class . ':Driver'])->group(function () {
    ...
    // Driver Route Assignments & Schedule
    Route::get('/assignments/today', [RouteAssignmentController::class, 'driverTodayAssignments'])->name('assignments.today');
    Route::get('/assignments', [RouteAssignmentController::class, 'driverAssignments'])->name('assignments.index');
    Route::get('/assignments/history', [RouteAssignmentController::class, 'driverAssignmentHistory'])->name('assignments.history');
    Route::get('/assignments/{assignment}', [RouteAssignmentController::class, 'driverAssignmentShow'])->name('assignments.show');

    // Driver Performance - redirect to new module dashboard
    Route::get('/performance', function () {
        return redirect()->route('driver-performance.index');
    })->name('performance.index');

    // Route Assignment Actions (for drivers to update their assignments)
    Route::post('/assignments/{assignment}/start', [RouteAssignmentController::class, 'driverStartRoute'])->name('assignments.start');
    Route::post('/assignments/{assignment}/complete', [RouteAssignmentController::class, 'driverCompleteRoute'])->name('assignments.complete');
    Route::post('/assignments/{assignment}/update-location', [RouteAssignmentController::class, 'driverUpdateLocation'])->name('assignments.update-location');
    Route::post('/assignments/{assignment}/checkpoint/{checkpoint}/visit', [RouteAssignmentController::class, 'driverCheckpointVisit'])->name('assignments.checkpoint.visit');
});
...
Route::prefix('maintenance')->name('maintenance.')->middleware('auth')->group(function () {
    ...
    Route::middleware([RoleMiddleware::class . ':Admin'])->group(function () {
        ...
        // Live route API endpoints (for AJAX / GPS updates)
        Route::post('/api/route/{route}/position', [\App\Http\Controllers\RouteManagement\LiveRouteController::class, 'updatePosition'])->name('api.route.position')->middleware('auth');
        Route::get('/api/route/{route}/data', [\App\Http\Controllers\RouteManagement\LiveRouteController::class, 'getRouteData'])->name('api.route.data')->middleware('auth');
    });
...
Route::prefix('route-management')->name('route-management.')->middleware(['auth', RoleMiddleware::class . ':Admin|Technician'])->group(function () {
    Route::get('/', [\App\Http\Controllers\RouteManagement\RouteController::class, 'index'])->name('index');
    Route::get('/create', [\App\Http\Controllers\RouteManagement\RouteController::class, 'create'])->name('create');
    Route::post('/', [\App\Http\Controllers\RouteManagement\RouteController::class, 'store'])->name('store');
    Route::get('/{route}', [\App\Http\Controllers\RouteManagement\RouteController::class, 'show'])->name('show');
    Route::get('/{route}/edit', [\App\Http\Controllers\RouteManagement\RouteController::class, 'edit'])->name('edit');
    Route::put('/{route}', [\App\Http\Controllers\RouteManagement\RouteController::class, 'update'])->name('update');
    Route::delete('/{route}', [\App\Http\Controllers\RouteManagement\RouteController::class, 'destroy'])->name('destroy');
    Route::get('/vehicles', [\App\Http\Controllers\RouteManagement\RouteController::class, 'vehicles'])->name('vehicles');
    Route::get('/active-routes', [\App\Http\Controllers\RouteManagement\RouteController::class, 'activeRoutes'])->name('active-routes');
});

/* -------------------- ROUTE ASSIGNMENTS MODULE -------------------- */
Route::prefix('route-assignments')->name('route-assignments.')->middleware(['auth', RoleMiddleware::class . ':Admin|Technician'])->group(function () {
    ...
    Route::put('/{routeAssignment}', [RouteAssignmentController::class, 'update'])->name('update');
    Route::delete('/{routeAssignment}', [RouteAssignmentController::class, 'destroy'])->name('destroy');
    Route::post('/{routeAssignment}/start', [RouteAssignmentController::class, 'start'])->name('start');
    Route::post('/{routeAssignment}/complete', [RouteAssignmentController::class, 'complete'])->name('complete');
    Route::post('/{routeAssignment}/cancel', [RouteAssignmentController::class, 'cancel'])->name('cancel');
    Route::get('/{routeAssignment}/tracking', [RouteAssignmentController::class, 'tracking'])->name('tracking');
    Route::post('/{routeAssignment}/update-location', [RouteAssignmentController::class, 'updateLocation'])->name('update-location');
    Route::post('/bulk-assign', [RouteAssignmentController::class, 'bulkAssign'])->name('bulk-assign');
    Route::get('/calendar', [RouteAssignmentController::class, 'calendar'])->name('calendar');
});

/* -------------------- DRIVER PERFORMANCE MODULE -------------------- */
Route::prefix('driver-performance')->name('driver-performance.')->middleware(['auth'])->group(function () {
    Route::get('/', [\App\Http\Controllers\DriverPerformance\DashboardController::class, 'index'])->name('index');
    Route::get('/dashboard', [\App\Http\Controllers\DriverPerformance\DashboardController::class, 'index'])->name('dashboard');

    Route::middleware([RoleMiddleware::class . ':Admin|Technician'])->group(function () {
        Route::get('/drivers/{driver}/edit', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'edit'])->name('edit');
        Route::put('/drivers/{driver}', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'update'])->name('update');
        Route::get('/drivers/{driver}/analyze-behavior', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'analyzeBehavior'])->name('analyze-behavior');
        Route::get('/drivers/{driver}/metrics/{metric}/edit', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'editMetric'])->name('metrics.edit');
        Route::put('/drivers/{driver}/metrics/{metric}', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'updateMetric'])->name('metrics.update');
    ...
    Route::middleware([RoleMiddleware::class . ':Driver'])->group(function () {
        Route::get('/profile', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'profile'])->name('profile');
    });

    // Shared routes (accessible by all authenticated users)
    Route::get('/drivers/{driver}', [\App\Http\Controllers\DriverPerformance\DriverController::class, 'show'])->name('show');
});
