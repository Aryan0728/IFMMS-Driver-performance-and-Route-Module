...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-edit text-primary"></i> Edit Route: {{ $route->name }}
        </h1>
    ...
    <div class="card border-left-warning shadow mb-4">
        <div class="card-header py-3 bg-warning text-white">
            <h6 class="m-0 font-weight-bold">
                <i class="fas fa-map-marked-alt"></i> Edit Route Details & Waypoints
            </h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('route-management.update', $route) }}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required value="{{ old('name', $route->name) }}" placeholder="e.g., Downtown Delivery Route">
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('assigned_driver_id') is-invalid @enderror" id="assigned_driver_id" name="assigned_driver_id" required>
                                ...
                                <option value="{{ $driver->id }}" {{ old('assigned_driver_id', $route->assigned_driver_id) == $driver->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" required value="{{ old('start_time', $route->start_time->format('Y-m-d\TH:i')) }}">
                            @error('start_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control @error('estimated_end_time') is-invalid @enderror" id="estimated_end_time" name="estimated_end_time" required value="{{ old('estimated_end_time', $route->estimated_end_time->format('Y-m-d\TH:i')) }}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('estimated_distance') is-invalid @enderror" id="estimated_distance" name="estimated_distance" step="0.1" min="0" required value="{{ old('estimated_distance', $route->estimated_distance) }}" placeholder="e.g., 45.5">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('estimated_duration') is-invalid @enderror" id="estimated_duration" name="estimated_duration" min="1" required value="{{ old('estimated_duration', $route->estimated_duration) }}" placeholder="e.g., 120">
                ...
                <div class="form-group">
                    <label for="optimized_waypoints" class="form-label">
                    ...
                    <textarea class="form-control @error('optimized_waypoints') is-invalid @enderror" id="optimized_waypoints" name="optimized_waypoints" rows="6" required readonly placeholder='Waypoints will appear here as JSON'>{{ old('optimized_waypoints', json_encode($route->optimized_waypoints, JSON_PRETTY_PRINT)) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Any additional notes about this route...">{{ old('notes', $route->notes) }}</textarea>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var map = L.map('waypoints-map').setView([-17.7134, 178.0650], 8); // Fiji center
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 18,
        attribution: '© OpenStreetMap contributors'
    }).addTo(map);
    var waypoints = {!! json_encode($route->optimized_waypoints ?? []) !!};
    var markers = [];

    function updateWaypointsTextarea() {
        document.getElementById('optimized_waypoints').value = JSON.stringify(waypoints, null, 2);
    }
    ...
    if (Array.isArray(waypoints)) {
        waypoints.forEach(function(waypoint, index) {
            if (waypoint.lat && waypoint.lng) {
                var marker = L.marker([waypoint.lat, waypoint.lng], { draggable: true }).addTo(map);
                marker.bindPopup(waypoint.name || 'Waypoint ' + (index + 1)).openPopup();
                markers.push(marker);
