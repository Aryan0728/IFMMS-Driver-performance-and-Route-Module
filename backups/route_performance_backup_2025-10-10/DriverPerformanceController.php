...
class DriverPerformanceController extends Controller
{
    ...
    /**
     * Show performance dashboard for the authenticated driver
     */
    public function driverPerformance(Request $request)
    {
        ...
        // Get driver's assignments for the period
        $assignments = RouteAssignment::where('driver_id', $driver->id)
            ->whereBetween('assignment_date', [$startDate, $endDate])
            ->with(['route', 'vehicle', 'checkpointVisits'])
            ->get();
        ...
        $metrics = [
            ...
            'total_distance' => $assignments->where('status', 'completed')->sum('actual_distance'),
            'total_fuel_consumed' => $assignments->where('status', 'completed')->sum('fuel_consumed'),
            'avg_fuel_efficiency' => $this->calculateFuelEfficiency($assignments->where('status', 'completed')),
            'on_time_percentage' => $this->calculateDriverOnTimePercentage($assignments->where('status', 'completed')),
            'total_duration' => $assignments->where('status', 'completed')->sum('actual_duration'),
            'avg_duration' => $assignments->where('status', 'completed')->avg('actual_duration')
        ];

        // Get recent assignments
        $recentAssignments = RouteAssignment::where('driver_id', $driver->id)
            ->with(['route', 'vehicle'])
            ->orderBy('assignment_date', 'desc')
            ->limit(10)
            ->get();
        ...
        $trends = $this->getDriverPerformanceTrends($driver->id);

        // Get upcoming assignments
        $upcomingAssignments = RouteAssignment::where('driver_id', $driver->id)
        ...
        return view('driver.performance.index', compact(
            'driver',
            'metrics',
            'recentAssignments',
            'trends',
    ...
    /**
     * Calculate fuel efficiency for completed assignments
     */
    private function calculateFuelEfficiency($completedAssignments)
    {
        $totalDistance = $completedAssignments->sum('actual_distance');
        $totalFuel = $completedAssignments->sum('fuel_consumed');

        if ($totalFuel > 0) {
            return round($totalDistance / $totalFuel, 2); // km per liter
        }
    ...
    /**
     * Calculate on-time percentage for a specific driver
     */
    private function calculateDriverOnTimePercentage($completedAssignments)
    {
        if ($completedAssignments->isEmpty()) {
            return 0;
        }

        $onTimeCount = $completedAssignments->filter(function ($assignment) {
            if (!$assignment->actual_end_time || !$assignment->scheduled_end_time) {
                return false;
            }

            $scheduledEnd = Carbon::parse($assignment->assignment_date->format('Y-m-d') . ' ' . $assignment->scheduled_end_time);
            $actualEnd = Carbon::parse($assignment->assignment_date->format('Y-m-d') . ' ' . $assignment->actual_end_time);

            return $actualEnd->lte($scheduledEnd->addMinutes(15)); // 15 minutes grace period
        })->count();

        return round(($onTimeCount / $completedAssignments->count()) * 100, 1);
    }

    /**
     * Get performance trends for a specific driver
     */
    private function getDriverPerformanceTrends($driverId)
    {
        $trends = [];
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $assignments = RouteAssignment::where('driver_id', $driverId)
                ...
                ->whereMonth('assignment_date', $month->month)
                ->get();
            ...
            $trends[] = [
                ...
                'completion_rate' => $assignments->count() > 0 ?
                    round(($completed->count() / $assignments->count()) * 100, 1) : 0,
                'total_distance' => $completed->sum('actual_distance'),
                'on_time_percentage' => $this->calculateDriverOnTimePercentage($completed)
            ];
        }
